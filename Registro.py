...
def conectar_bd():
    """
    Esta función establece la conexión a la base de datos MySQL.
    Modifica los parámetros (host, user, password, database) según tu configuración.
...
def validar_telefono(telefono):
    ...
    Utiliza una expresión regular para verificar si el número es válido.
...
def validar_correo(email):
    ...
    Utiliza una expresión regular para verificar si el formato es válido.
    Devuelve True si es válido, False en caso contrario.
    """
    return bool(re.match(r"[^@]+@[^@]+\.[^@]+", email))


# Crear un contenedor visual en la interfaz de Streamlit
with st.container():
    # Título y descripción en formato HTML centrado
    ...
    # Recopilación de datos del usuario en dos columnas
    ...
    # Botón para registrar proveedor en la base de datos
    if st.button("Registrar Proveedor", key="registrar_proveedor"):
        ...
        if not NombreProveedor or not Ubicacion:
            ...
            # Conexión a la base de datos y registro del proveedor
    ...
    # Expander para mostrar la tabla de proveedores registrados
    with st.expander("Ver Proveedores Registrados"):
        try:
            with conectar_bd() as conexion:
                try:
                    with conexion.cursor() as cursor:
                        # Consulta para obtener datos de proveedores desde la base de datos
                    ...
                    # Botón para eliminar la última fila de proveedores y productos relacionados
                    if st.button("Eliminar Última Fila de Proveedores", key="eliminar_ultima_fila_proveedores"):
                        if not df.empty:
                            ...
                            try:
                                with conexion.cursor() as cursor:
                                    # Consultas para eliminar proveedor y productos relacionados
...
# Separador horizontal en la interfaz
st.write("---")
...
with st.container():
    ...
    # Selección del proveedor mediante un cuadro desplegable
    ...
    # Botón para registrar producto en la base de datos
    if st.button("Registrar Producto", key="registrar_producto"):
        if not Proveedor_ID or not Nombre or not PrecioCompra or not PrecioVenta or not Stock:
            ...
            # Conexión a la base de datos y registro del producto
            with conectar_bd() as conexion:
                cursor = conexion.cursor()
    ...
    # Sección para mostrar los productos en una tabla expandible
    with st.expander("Ver Productos Registrados"):
        try:
            with conectar_bd() as conexion:
                with conexion.cursor() as cursor:
                    # Consulta para obtener datos de productos y proveedores desde la base de datos
                    query = """
                    ...
                    # Botón para eliminar la última fila de productos y existencias relacionadas
